...
import { matchesQuery, extractSnippet, applyFilters } from "./search";
...
describe("matchesQuery", () => {
	...
	it("matches title", () => {
		expect(matchesQuery(makeVisit({ title: "React Hooks Guide" }), "react")).toBe(true);
	});
	...
	it("matches textContent", () => {
		expect(
			matchesQuery(
				makeVisit({ textContent: "This article discusses TypeScript generics" }),
				"generics",
			),
		).toBe(true);
	});
	...
	it("is case insensitive", () => {
		expect(matchesQuery(makeVisit({ title: "REACT Hooks" }), "react")).toBe(true);
	});
...
describe("applyFilters", () => {
	const visits = [
		makeVisit({ id: 1, domain: "github.com", duration: 120, timestamp: Date.now() }),
		makeVisit({ id: 2, domain: "stackoverflow.com", duration: 30, timestamp: Date.now() - 1000 }),
		makeVisit({
			id: 3,
			domain: "github.com",
			duration: 60,
		...
		makeVisit({ id: 4, domain: "youtube.com", duration: 300, timestamp: Date.now() - 3000 }),
	];
