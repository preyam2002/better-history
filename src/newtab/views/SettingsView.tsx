...
export const SettingsView = () => {
	...
	return (
		<div className="max-w-lg space-y-6">
			{/* Session Gap */}
			<SettingsSection title="Session Detection" description="A new browsing session starts after this period of inactivity.">
			...
			{/* Excluded Domains */}
			<SettingsSection title="Excluded Domains" description="Pages from these domains won't be tracked or stored.">
			...
			{/* Auto-Prune */}
			<SettingsSection title="Storage Management" description="Automatically delete extracted page content older than a set period to save space. Visit metadata (title, URL, duration) is always kept.">
				<div className="flex gap-2">
					{([
			...
			<SettingsSection title="Data">
				{stats && (
					<div className="grid grid-cols-3 gap-3 mb-4">
						<DataStat value={stats.visitCount.toLocaleString()} label="visits" />
						<DataStat value={stats.sessionCount.toLocaleString()} label="sessions" />
...
const SettingsSection = ({
	...
	<section className="bg-white dark:bg-sand-900/50 rounded-xl border border-sand-200 dark:border-sand-800 p-5">
		<h3 className="text-[13px] font-semibold text-sand-700 dark:text-sand-200 mb-1">{title}</h3>
		{description && (
...
const DataStat = ({ value, label }: { value: string; label: string }) => (
	<div className="text-center p-3 bg-sand-50 dark:bg-sand-850 rounded-lg">
		...
		<div className="text-[10px] text-sand-400 tracking-wide uppercase">{label}</div>
	</div>
);
