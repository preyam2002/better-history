...
const SummaryCard = ({
	...
	<div
		className={`bg-white dark:bg-sand-900/50 rounded-xl border p-4 transition-shadow hover:shadow-sm ${
			accent === "honey"
	...
	>
		...
		<div className="text-[10px] text-sand-400 mt-1 tracking-wide uppercase">
			{label}
		</div>
	</div>
);
	...
	<div className="space-y-6 animate-pulse">
		<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
			{Array.from({ length: 4 }).map((_, i) => (
				<div
					key={i}
					className="bg-white dark:bg-sand-900/50 rounded-xl border border-sand-200 dark:border-sand-800 p-4 h-20"
				>
		...
		<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
			{Array.from({ length: 2 }).map((_, i) => (
				<div
					key={i}
					className="bg-white dark:bg-sand-900/50 rounded-xl border border-sand-200 dark:border-sand-800 p-4 h-64"
				/>
			))}
		</div>
	</div>
