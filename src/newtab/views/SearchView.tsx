...
export const SearchView = () => {
	...
	const flatResults = useMemo(() => (results ? results : []), [results]);

	// biome-ignore lint/correctness/useExhaustiveDependencies: results is the intentional trigger
	useEffect(() => {
		setSelectedIndex(-1);
	}, [results]);
	...
	useEffect(() => {
		...
		const el = listRef.current.querySelector(
			`[data-visit-id="${flatResults[selectedIndex]?.id}"]`,
		);
	...
	const bulkDelete = useCallback(async () => {
		...
		// Refresh affected sessions (removes empty ones, updates counts)
		await Promise.all(sessionIds.map(refreshSession));
		toast(
			`${selectedIds.size} visit${selectedIds.size > 1 ? "s" : ""} removed`,
			"info",
		);
		clearSelection();
	...
	const bulkExport = useCallback(async () => {
		...
		toast(
			`Exported ${selectedIds.size} visit${selectedIds.size > 1 ? "s" : ""}`,
		);
	}, [selectedIds, toast]);
					...
					{selectionMode && (
						<div className="flex items-center gap-2 px-3 py-2.5 bg-sand-100 dark:bg-sand-850 rounded-lg animate-in">
							<button
								type="button"
								onClick={
									selectedIds.size === flatResults.length
										? clearSelection
										: selectAll
								}
							...
							<div className="space-y-0.5">
								{visits.map((visit) => (
									<VisitCard
										...
										selected={
											!selectionMode &&
											flatResults[selectedIndex]?.id === visit.id
										}
										onSelect={selectionMode ? undefined : setDetailVisit}
										checked={
											selectionMode ? selectedIds.has(visit.id!) : undefined
										}
