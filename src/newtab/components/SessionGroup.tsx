...
import { formatDuration, formatTime, getFaviconUrl } from "../../shared/utils";
...
export const SessionGroup = ({ session }: SessionGroupProps) => {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: visits.length triggers height recalc
	useEffect(() => {
	...
	const topFavicons = useMemo(() => {
		...
		for (const v of session.visits) {
			if (!seen.has(v.domain)) {
				seen.add(v.domain);
				icons.push({
					url: getFaviconUrl(v.favIconUrl, v.domain),
					domain: v.domain,
				});
				...
				<div className="flex items-center gap-3 min-w-0">
					...
					<div className="min-w-0">
						...
						<div className="text-[11px] text-sand-400 truncate mt-0.5">
							...
							{uniqueDomains.length > 3 && ` +${uniqueDomains.length - 3} more`}
						</div>
					</div>
				</div>
